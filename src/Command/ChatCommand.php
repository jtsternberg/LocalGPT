...
class ChatCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Chatting with ' . $gptConfig['title']);

        $apiKey = $configService->getApiKey($gptConfig['provider']);
        if (!$apiKey) {
            $io->error('API key for ' . $gptConfig['provider'] . ' not found in .env file.');
            return Command::FAILURE;
        }

        $provider = new GeminiProvider($apiKey, $gptConfig['model']);
        $gptDir = getcwd() . '/' . $name;
        ...
        $io->success('You can start chatting now. Type "exit" to quit.');
